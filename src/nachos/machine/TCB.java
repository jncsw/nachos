...
public final class TCB {
	 ...
	 */
	     ...
	     */
            tcbTarget = new Runnable() {
                public void run() {
                    threadroot();
                }//如果是第一个进程，则初始化并运行，否则，先执行yield等待上下文切换
            };
	     ...
	     */ //java线程尚未开始，但我们需要把它堵在yield()。我们通过暂时关闭当前TCB，启动新的java线程，并等待它唤醒我们从threadroot()。一旦新的TCB将我们唤醒，上下文切换到新的TCB是安全的。
     ...
     * current TCB.
     */
    public void contextSwitch() {//上下文切换
	 ...
	 */
        ...
        this.interrupt();//激活新进程，标记running=1 && notify  //this 为nextThread
        previous.yield();//yield当前进程
    }
